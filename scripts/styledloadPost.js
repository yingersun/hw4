...
function loadPost(post) {
  let post_html = `<span>Title: <span>${post.title}</span> (<span>${post.date}</span>)</span> <span>Summary: <span>${post.summary}</span></span><div><button><i class="fas fa-edit"></i>Edit</button><button><i class="fas fa-trash"></i>Delete</button></div>`;
  return post_html;
}
